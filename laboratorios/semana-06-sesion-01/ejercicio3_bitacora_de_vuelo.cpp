 ...
 private:
    ...
    public:
        BitacoraDeVuelo(int nuevaCapacidad) {
            ...
            consumodecombustible = new double[capacidad];
            std::cout << "Bitacora creada para " << capacidad << " tramos" << std::endl;
        }
            BitacoraDeVuelo(BitacoraDeVuelo&& otro){


            consumodecombustible = otro.consumodecombustible;
            capacidad= otro.capacidad;

            otro.consumodecombustible = nullptr;
            otro.capacidad = 0;
        }
        ...
        void guardarConsumo(int indice, double valor) {
            consumodecombustible[indice] = valor;
        }

        double getConsumo(int indice) {
            if (consumodecombustible == nullptr) {
                std::cout << "Bitacora vacia(fue movida)" << std::endl;
                return 0.0;
            }
            return consumodecombustible[indice];
        }
